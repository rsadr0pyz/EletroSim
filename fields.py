...
class EFieldGuide:
    ...
    def calculate_field_profile(self):
        ...
        ez = np.sin(x*e_k_x) * np.sin(y*e_k_y) * np.exp(-z*e_gam) 
        ex = -e_gam / h2 * e_k_x * np.cos(x*e_k_x) * np.sin(y*e_k_y) * np.exp(-z*e_gam)
        ey = -e_gam / h2 * e_k_y * np.sin(x*e_k_x) * np.cos(y*e_k_y) * np.exp(-z*e_gam)
