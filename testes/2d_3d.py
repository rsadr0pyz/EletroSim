...
dt = ds/(2 * c0)
jmax = int(SPACE_SIDE_LENGTH  / ds)+1
...
X = np.linspace(0, jmax-1, jmax)
Y = np.linspace(0, jmax-1, jmax)
xy, yx = np.meshgrid(X, Y)
Explot = ax.plot_surface(xy, yx, ez_field, cmap='viridis')
ax.set_zlim([-0.2, 0.2])
...
animation = ani.FuncAnimation(fig=fig, func=update, frames=SIMULATION_TIME * FPS, interval=1000/FPS, repeat=False)

animation.save("out.gif")